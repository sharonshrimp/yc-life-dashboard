...
export default function FitnessDashboard() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center font-black text-indigo-500 italic">
      ...
      <div className="animate-pulse tracking-widest text-sm">YC_DASHBOARD INITIALIZING...</div>
    </div>
  );
    ...
    <main className="min-h-screen bg-[#F8FAFC] pb-24 font-sans text-slate-900 overflow-x-hidden">
      ...
      <header className="pt-16 pb-8 px-8 max-w-md mx-auto animate-in fade-in slide-in-from-bottom-4 duration-1000">
        <div className="relative">
          ...
          <h1 className="text-5xl font-black tracking-tighter leading-none italic bg-gradient-to-r from-slate-900 via-indigo-800 to-slate-900 bg-clip-text text-transparent">
      ...
      {/* 導航欄 */}
      <nav className="sticky top-0 z-50 bg-[#F8FAFC]/60 backdrop-blur-xl px-4 py-4">
...
function NavButton({ active, onClick, icon, label }: NavButtonProps) {
  return (
    <button 
      onClick={onClick}
      className={`flex-1 flex items-center justify-center gap-2 py-4 rounded-[2rem] transition-all duration-500 ${
        active 
